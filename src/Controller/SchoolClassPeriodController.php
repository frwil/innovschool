...
final class SchoolClassPeriodController extends AbstractController
{
    ...
    public function new(
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!$numberingType) {
                $name = $request->request->get('name');
                if ($name && $sectionCategory && $period && $school) {
                    $class = new SchoolClassPeriod();
                    $class->setSchool($school);
                    $class->setPeriod($period);
                    $class->setEvaluationAppreciationTemplate($school->getEvaluationAppreciationTemplate());
        ...
        if (!in_array('ROLE_ADMIN',$this->getUser()->getRoles()) && !in_array('ROLE_SUPER_ADMIN',$this->getUser()->getRoles())) {
            $sectionIds = $entityManager->getRepository(UserBaseConfiguration::class)
                ->findOneBy(['user' => $this->getUser()])
                ->map(fn(UserBaseConfiguration $ubc) => $ubc->getSectionList())
                ->flatten()
                ->toArray();
        } else {
            $sectionIds = $entityManager->getRepository(StudyLevel::class)->findAll();
    ...
    public function getClassesBySection(SessionInterface $session,Request $request, SchoolClassPeriodRepository $schoolClassRepository, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        $sectionCategories = $entityManager->getRepository(Classe::class)->findBy(['studyLevel' => $sectionId]);
        $classOccurences = [];
        foreach ($sectionCategories as $classe) {
            $c=array_map(function (ClassOccurence $classOccurence) {
                return $classOccurence->getId();
            }, $classe->getClassOccurences()->toArray());
            foreach ($c as $cId) {
                if (!in_array($cId, $classOccurences)) {
                    $classOccurences[] = $cId;
                }
            }
        }
        ...
        if (in_array('ROLE_SUPER_ADMIN', $this->getUser()->getRoles())) {
            // Récupérer les classes associées à la section
            $classes = $schoolClassRepository->findBy(['classOccurence' => $classOccurences, 'period' => $period, 'school' => $this->currentSchool]);
        } else {
            $config = $this->getUser()->getBaseConfigurations()->toArray();
            if (count($config) > 0) {
                $classes = $schoolClassRepository->findBy(['classOccurence' => $classOccurences, 'id' => $config[0]->getClassList(), 'period' => $period, 'school' => $this->currentSchool]);
            } else {
                $classes = [];
            }
        }
    ...
    public function getGenericClassesBySection(Request $request, SchoolClassPeriodRepository $schoolClassRepository, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        if (in_array('ROLE_SUPER_ADMIN',$this->getUser()->getRoles())) {
            ...
            $config = $this->getUser()->getBaseConfigurations()->toArray();
            if (count($config) > 0) {
                $classes = array_filter($classes, function ($class) use ($config) {
                    return in_array($class->getId(), $config[0]->getClassList());
                });
            } else {
                $classes = [];
            }
    ...
    public function getClassesBySectionWithStudents(SessionInterface $session,Request $request, SchoolClassPeriodRepository $schoolClassRepository, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        $this->currentPeriod = $this->entityManager->getRepository(SchoolPeriod::class)->find($this->session->get('period_id'));
            ...
            $config = $this->getUser()->getBaseConfigurations()->toArray();
            if (count($config) > 0) {
                $classes = $schoolClassRepository->findBy(['classOccurence' => $config[0]->getClassList()]);
                $classes = array_filter($classes, function (SchoolClassPeriod $class) {
                    return $class->getStudentClasses()->count() > 0; // Filtrer les classes avec des étudiants
                });
            } else {
                $classes = [];
            }
    ...
    public function edit(SessionInterface $session,Request $request, SchoolClassPeriod $class, EntityManagerInterface $entityManager, SluggerInterface $slugger, OperationLogger $operationLogger): Response
    {
        if ($request->isMethod('POST')) {
            ...
            $operationLogger->log(
                ...
                $class->getClassOccurence()->getId(), // ID de l'entité
                null,           // Pas d'erreur
                ['name' => $class->getClassOccurence()->getName()] // Données additionnelles (optionnel)
            );
    ...
    public function delete(
        ...
        SchoolClassPeriod $class,
        EntityManagerInterface $entityManager,
        OperationLogger $operationLogger // <-- Ajoute l'injection du service ici
    ): Response {
        if ($this->isCsrfTokenValid('delete' . $class->getId(), $request->request->get('_token'))) {
            $classId = $class->getClassOccurence()->getClasse()->getId();
            $className = $class->getClassOccurence()->getName();

            $entityManager->remove($class);
            $entityManager->remove($class->getClassOccurence());
            $entityManager->flush();
            ...
            $operationLogger->log(
                ...
                'SUCCESS',             // Statut
                'SchoolClassPeriod',         // Nom de l'entité
                $classId,              // ID de l'entité
                null,                  // Pas d'erreur
                ['name' => $className] // Données additionnelles
            );

            $this->addFlash('success', 'Classe supprimée avec succès.');
        }

        return $this->redirectToRoute('app_school_class_show', ['id' => $classId]);
    }
    ...
    public function editStudyLevel(
        ...
        if ($request->isMethod('POST')) {
            ...
            $operationLogger->log(
                'MODIFICATION', // Type d'opération
                'SUCCESS',      // Statut
                'StudyLevel', // Nom de l'entité
                $classe->getId(), // ID de l'entité
                null,           // Pas d'erreur
                ['name' => $classe->getName()] // Données additionnelles (optionnel)
            );
    ...
    #[Route('/import/excel', name: 'app_class_import_excel', methods: ['POST'])]
    public function importClassesExcel(SessionInterface $session,Request $request, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
    {
        $this->session = $session;
        $this->currentSchool = $this->entityManager->getRepository(School::class)->find($this->session->get('school_id'));
        $this->currentPeriod = $this->entityManager->getRepository(SchoolPeriod::class)->find($this->session->get('period_id'));
    ...
    #[Route('/import/json', name: 'app_class_import_json', methods: ['POST'])]
    public function importClassesJson(Request $request, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
    {
        /** @var UploadedFile $file */
        $file = $request->files->get('importFile');
        if (!$file) {
