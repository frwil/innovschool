    ...
    public function generateReceipt(SessionInterface $session,Request $request, NumberToWords $numberToWords,EntityManagerInterface $entityManager): Response
    {
        ...
        if (!is_array($paymentId)) {
            ...
            foreach ($totalPayments as $payment) {
                ...
                if (!isset($modalities[$modaliteLabel])) {
                    $modalities[$modaliteLabel] = [
                        ...
                        'totalPaymentAmount' => 0,
                        'totalRemainingAmount' => 0,
                        'totalReductionAmount' => array_sum(array_map(function($p){return $p->getReductionAmount();},$payment->getPaymentModal()->getAdmissionReductions()->toArray())),
                    ];
                }
                ...
                $modalities[$modaliteLabel]['totalPaymentAmount'] += $payment->getPaymentAmount();
                $modalities[$modaliteLabel]['totalRemainingAmount'] += $amountToPay - $payment->getPaymentAmount();
                $modalities[$modaliteLabel]['totalReductionAmount'] = array_sum(array_map(function($p){return $p->getReductionAmount();},$payment->getPaymentModal()->getAdmissionReductions()->toArray()));
                $i++;
            }
            $modals = $this->schoolClassPaymentModalRepository->findBy(['schoolClassPeriod' => $class, 'schoolPeriod' => $schoolPeriod, 'school' => $this->currentSchool]);
            foreach ($modals as $modal) {
                ...
                if (!isset($modalities[$modalLabel])) {
                    $modalities[$modalLabel] = [
                        ...
                        'totalPaymentAmount' => 0,
                        'totalRemainingAmount' => 0,
                        'totalReductionAmount' =>  array_sum(array_map(function($p){return $p->getReductionAmount();},array_filter($modal->getAdmissionReductions()->toArray(),function($p) use ($student) {
                            return $p->getStudent() && $p->getStudent()->getId() === $student->getId();
                        }))),
                    ];
                }
                ...
                $modalities[$modalLabel]['totalPaymentAmount'] += 0;
                $modalities[$modalLabel]['totalRemainingAmount'] = $modalities[$modalLabel]['totalAmountToPay'] - $modalities[$modalLabel]['totalPaymentAmount'];
                $modalities[$modalLabel]['totalReductionAmount'] =  array_sum(array_map(function($p){return $p->getReductionAmount();},array_filter($modal->getAdmissionReductions()->toArray(),function($p) use ($student) {
                    return $p->getStudent() && $p->getStudent()->getId() === $student->getId();
                })));
            }
            ...
            return $this->render('receipt/receipt' . ($paymentId != null ? '_individual' : '') . '.html.twig', [
                ...
                'class' => $class->getClassOccurence()->getName(),
                'payments' => is_array($payments) ? array_map(function ($payment) use ($modalities) {
                    return [
                        'modalite' => $payment->getPaymentModal()->getLabel(), // Modalité
                        'dueDate' => $payment->getPaymentModal()->getDueDate(), // Date d'échéance
                        'amountToPay' => $modalities[$payment->getPaymentModal()->getLabel()]['totalAmountToPay']-($payment->getPaymentModal()->getModalType()=='base' ? array_sum(array_map(function($p){return $p->getReductionAmount();},$payment->getPaymentModal()->getAdmissionReductions()->toArray())) : 0), // Montant à payer
                        'paymentAmount' => $payment->getPaymentAmount(), // Montant payé
                        'remainingAmount' => $payment->getPaymentModal()->getAmount() - $payment->getPaymentAmount(), // Reste à payer
                    ];
                }, $payments) : [
                    [
                        'modalite' => $payments->getPaymentModal()->getLabel(), // Modalité
                        'dueDate' => $payments->getPaymentModal()->getDueDate(), // Date d'échéance
                        'amountToPay' => $modalities[$payments->getPaymentModal()->getLabel()]['totalAmountToPay']-($payment->getPaymentModal()->getModalType()=='base' ? array_sum(array_map(function($p){return $p->getReductionAmount();},$payments->getPaymentModal()->getAdmissionReductions()->toArray())) : 0), // Montant à payer
                        'paymentAmount' => $payments->getPaymentAmount(), // Montant payé
                        'remainingAmount' => $payments->getPaymentModal()->getAmount() - $payments->getPaymentAmount(), // Reste à payer
                    ]
                ],
                ...
                'docTitle' => $paymentId != null ? 'RECU DE PAIEMENT' : 'RECAPITULATIF DES PAIEMENTS',
                'paymentDate' => $paymentId != null ? $payments[0]->getPaymentDate()->format('d/m/Y') : null,
                'totalPayments' => $paymentId != null ? array_map(function ($payment) {
                    return [
                        'modalite' => $payment->getPaymentModal()->getLabel(), // Modalité
                        'dueDate' => $payment->getPaymentModal()->getDueDate(), // Date d'échéance
                        'amountToPay' => $payment->getPaymentModal()->getAmount(), // Montant à payer
                        'paymentAmount' => $payment->getPaymentAmount(), // Montant payé
                        'remainingAmount' => $payment->getPaymentModal()->getAmount() - $payment->getPaymentAmount(), // Reste à payer
                        'reductionAmount' => array_sum(array_map(function($p){return $p->getReductionAmount();},$payment->getPaymentModal()->getAdmissionReductions()->toArray())), // Montant de la réduction
                    ];
                }, $totalPayments) : null,
                'amountInNumbers' => $paymentId != null ? strtoupper($numberToWords->convert($payments[0]->getPaymentAmount())) : null,
                'modalities' => $modalities,
                'totalAmountToPay' => $totalAmountToPay,
                'totalPaymentAmount' => $totalPaymentAmount,
                'totalRemainingAmount' => $totalRemainingAmount,
                'totalReductionAmount' => array_sum(array_map(function($p){return $p->getReductionAmount();},$payments[0]->getPaymentModal()->getAdmissionReductions()->toArray())),
                'currentUser' => $currentUser ? $currentUser->getFullName() : '',
            ...
            foreach ($allPayments as $payment) {
                $modal = $payment->getPaymentModal();
                $amountToPay = $modal->getAmount();
                $allReductions[$modal->getLabel()]= array_sum(array_map(function($p){return $p->getReductionAmount();},$modal->getAdmissionReductions()->toArray()));
