...
final class UserController extends AbstractController
{
    ...
    public function forcePasswordReset(Request $request, User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher, OperationLogger $operationLogger): Response
    {
        if ($request->isMethod('POST')) {
...
final class UserController extends AbstractController
{
    ...
    #[Route('/student/new', name: 'app_student_new', methods: ['POST'])]
    public function new(SessionInterface $session, Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        $this->session = $session;
        $this->entityManager = $entityManager;
        $this->currentSchool = $this->entityManager->getRepository(School::class)->find($this->session->get('school_id'));
    ...
    public function manage(EntityManagerInterface $entityManager): Response
    {
        ...
        if (in_array('ROLE_SUPER_ADMIN', $this->getUser()->getRoles())) {
            ...
            $config = $entityManager->getRepository(UserBaseConfiguration::class)->findBy(['user' => $user]);
            if (count($config) > 0) {
                $sections = $entityManager->getRepository(StudyLevel::class)->findBy(['id' => $config[0]->getSectionList()]);
                $classes = $entityManager->getRepository(SchoolClassPeriod::class)->findBy(['classOccurence' => $config[0]->getClassList()]);
            } else {
                $sections = [];
                $classes = [];
            }
    ...
    #[Route('/user/{id}/manage', name: 'app_user_manage_edit', methods: ['GET', 'POST'])]
    public function manageEdit(int $id, Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        $user = $entityManager->getRepository(User::class)->find($id);
        ...
        if ($configuration) {
            $sections = array_map(fn($section) => $section, $configuration->getSectionList());
            $classes = array_map(fn($class) => $class, $configuration->getClassList());
        }
    ...
    #[Route('/user/save-configuration', name: 'app_user_save_configuration', methods: ['POST'])]
    public function saveUserConfiguration(Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        // Décoder le contenu JSON de la requête
        $data = json_decode($request->getContent(), true);
    ...
    #[Route('/user/{id}/reset-password', name: 'app_user_reset_password', methods: ['POST'])]
    public function resetPassword(User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher): Response
    {
        // Nouveau mot de passe temporaire (exemple : 111111)
        $newPassword = '111111';
        $user->setPassword($passwordHasher->hashPassword($user, $newPassword));
    ...
    #[Route('/user/{id}/force-password-reset', name: 'app_force_password_reset', methods: ['GET', 'POST'])]
    public function forcePasswordReset(Request $request, User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher): Response
    {
        if ($request->isMethod('POST')) {
            $newPassword = $request->request->get('newPassword');
            $confirmPassword = $request->request->get('confirmPassword');
    ...
    #[Route('/teacher/new', name: 'app_teacher_new', methods: ['POST'])]
    public function teacherNew(SessionInterface $session, Request $request, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher): JsonResponse
    {
        $lastName = $request->request->get('lastName');
        $phone = $request->request->get('phone');
        $gender = $request->request->get('gender');
    ...
    #[Route('/student/import/excel', name: 'app_student_import_excel', methods: ['POST'])]
    public function importStudentsExcel(SessionInterface $session, Request $request, EntityManagerInterface $entityManager): Response
    {
        $this->session = $session;
        $this->entityManager = $entityManager;
        $this->currentSchool = $this->entityManager->getRepository(School::class)->find($this->session->get('school_id'));
    ...
    #[Route('/student/import/json', name: 'app_student_import_json', methods: ['POST'])]
    public function importStudentsJson(SessionInterface $session, Request $request, EntityManagerInterface $entityManager): Response
    {
        $this->session = $session;
        $this->currentSchool = $this->entityManager->getRepository(School::class)->find($this->session->get('school_id'));
        $this->currentPeriod = $this->entityManager->getRepository(SchoolPeriod::class)->find($this->session->get('period_id'));
