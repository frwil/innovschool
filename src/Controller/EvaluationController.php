...
class EvaluationController extends AbstractController
{
    ...
    private BulletinContextService $bulletinContextService;

    public function __construct(EntityManagerInterface $entityManager, OperationLogger $operationLog, BulletinDataService $bulletinDataService, BulletinContextService $bulletinContextService)
    {
        $this->entityManager = $entityManager;
        $this->operationLog = $operationLog;
        $this->bulletinDataService = $bulletinDataService;
        $this->bulletinContextService = $bulletinContextService;
    }
    ...
    public function getModules(ClassSubjectModuleRepository $moduleRepo, EvaluationRepository $evaluationRepo): JsonResponse
    {
...
class EvaluationController extends AbstractController
{
    ...
    public function index(
        ...
        if (in_array('ROLE_SUPER_ADMIN', $this->getUser()->getRoles())) {
            ...
            if (count($config) > 0) {
                $sections = $sectionRepo->findBy(['id' => $config[0]->getSectionList()]);
            } else {
                $sections = []; // Si aucune section n'est trouvée, on initialise un tableau vide
            }
        }
    ...
    public function getSubjectsByClass(SessionInterface $session, Request $request, StudySubjectRepository $subjectRepo, SchoolClassSubjectRepository $subjectClassRepo, SchoolPeriodRepository $periodRepo): JsonResponse
    {
        ...
        $classe= $this->entityManager->getRepository(SchoolClassPeriod::class)->findBy(['classOccurence' => $classId, 'school' => $this->currentSchool, 'period' => $this->currentPeriod]);
    ...
    #[Route('/add-module', name: 'app_add_module', methods: ['POST'])]
    public function addModule(Request $request, SubjectsModulesRepository $moduleRepo, EntityManagerInterface $entityManager): JsonResponse
    {
        $name = $request->request->get('name');
        $description = $request->request->get('description');
        ...
        $module = new SubjectsModules();
        $module->setModuleName($name);
        $module->setModuleSlug($description);
        // Les dates createdAt/updatedAt sont gérées automatiquement par les callbacks Doctrine
    ...
    public function mapModule(SessionInterface $session, Request $request, SubjectsModulesRepository $subjectsModulesRepo, EntityManagerInterface $entityManager, SchoolPeriodRepository $periodRepo, ClassSubjectModuleRepository $classSubjectModuleRepo): JsonResponse
    {
        ...
        $class = $this->entityManager->getRepository(SchoolClassPeriod::class)->findOneBy(['classOccurence' => $classId, 'school' => $this->currentSchool, 'period' => $this->currentPeriod]);
        if (!$class) {
            return new JsonResponse(['error' => 'Classe non trouvée.'], 404);
        }
        ...
        // Boucle sur chaque matière
        foreach ($subjectIds as $subjectId) {
            $subject = $this->entityManager->getRepository(SchoolClassSubject::class)->findOneBy(['studySubject'=>$subjectId]);
            if (!$subject) {
                return new JsonResponse(['error' => "Matière avec l'ID $subjectId non trouvée."], 404);
            }
            $moduleMapping = $classSubjectModuleRepo->findOneBy(['class' => $class, 'subject' => $subject, 'module' => $moduleSubject, 'period' => $period, 'school' => $this->currentSchool]);
            if (!$moduleMapping) {
                ...
                $module = new ClassSubjectModule();
                $module->setClass($class);
                $module->setSubject($subject->getStudySubject());
                $module->setModule($moduleSubject);
                $module->setSchool($this->currentSchool);
                $module->setModuleNotation((int) $moduleNotation);
                $module->setUpdatedAt(new \DateTime());
            ...
            $entityManager->flush();
            $standardModule = $subjectsModulesRepo->findOneBy(['moduleName' => 'Standard/Ecrit']);
            if ($standardModule) {
                $standardModuleMapping = $classSubjectModuleRepo->findOneBy(['class' => $class, 'subject' => $subject->getStudySubject(), 'module' => $standardModule, 'period' => $period, 'school' => $this->currentSchool]);
                if (!$standardModuleMapping) {
                    $standardModuleMapping = new ClassSubjectModule();
                    $standardModuleMapping->setClass($class);
                    $standardModuleMapping->setSubject($subject->getStudySubject());
                    $standardModuleMapping->setModule($standardModule);
                    $standardModuleMapping->setSchool($this->currentSchool);
                    $standardModuleMapping->setModuleNotation((int) $moduleNotation);
                    $standardModuleMapping->setUpdatedAt(new \DateTime());
    ...
    public function getModules(ClassSubjectModuleRepository $moduleRepo, EvaluationRepository $evaluationRepo): JsonResponse
    {
        ...
        foreach ($modules as $module) {
            ...
            $data[] = [
                ...
                'moduleName' => $module->getModule()->getModuleName(),
                'moduleNotation' => $module->getModuleNotation(),
                'evaluationCount' => $evaluationCount, // Nombre de classes ayant des évaluations
            ];
        }
        return new JsonResponse($data);
    }
    ...
    public function createEvaluation(
        ...
        // Récupérer les entités associées
        $classes = $classRepo->findBy(['classOccurence' => $classIds, 'school' => $this->currentSchool, 'period' => $this->currentPeriod]);
        if (!$classes) {
            return new JsonResponse(['error' => 'Classe non trouvée.'], 404);
        }
    ...
    public function bordereauGeneral(
        ...
        // Récupérer les entités associées
        $period = $this->currentPeriod;
        $class = $classRepo->findBy(['classOccurence' => $classId, 'period' => $period, 'school' => $this->currentSchool]);
        $evaluationTime = $timeRepo->find($evaluationTimeId);
        ...
        // Récupérer les évaluations si les notes doivent être incluses
        $evaluations = [];
        if ($includeNotes) {
            $evaluations = $evaluationRepo->findBy([
                'classSubjectModule' => $modules,
        ...
        return $this->render('evaluation/bordereau.general.html.twig', [
            ...
            'students' => $students, // Liste des élèves triés
            'classSubjectModules' => $classSubjectModules, // Modules associés aux matières
            'evaluations' => $evaluations,
            'includeNotes' => $includeNotes,
            'school' => $this->currentSchool,
        ]);
    ...
    #[Route('/module/{id}/delete', name: 'app_delete_module', methods: ['DELETE'])]
    public function deleteModule(int $id, ClassSubjectModuleRepository $moduleRepo, EvaluationRepository $evaluationRepo, EntityManagerInterface $entityManager): JsonResponse
    {
        $module = $moduleRepo->find($id);
        ...
        // Vérifier s'il existe des évaluations liées à ce module
        $evaluations = $evaluationRepo->findBy(['classSubjectModule' => $module]);

        if ($evaluations) {
            foreach ($evaluations as $evaluation) {
                $entityManager->remove($evaluation); // Supprimer chaque évaluation liée
            }
        }
    ...
    public function updateEvaluation(
        ...
        $moduleId = $request->request->get('moduleId');
        $value = $request->request->get('value');
        $timeId = $request->request->get('timeId'); // ID de la période d'évaluation
        $classId = $request->request->get('classId'); // ID de la classe
        ...
        if (!$evaluation) {
            //si l'évaluation n'existe pas, on la crée
            $evaluation = new Evaluation();
            $evaluation->setStudent($student);
            $evaluation->setClassSubjectModule($module);
            $evaluation->setTime($schoolEvaluationTime);
    ...
    public function createEvaluationAppreciationTemplate(
        ...
        $this->operationLog->log(
            ...
            $template->getId(),
            null,
            ['name' => $template->getName(), 'default' => $template->isEnabled()]
        );
    ...
    public function getStudentClassAttendance(
        ...
        // Récupérer l'entité StudentClass
        $classPeriods = $classRepo->findBy(['classOccurence' => $classId, 'period' => $this->currentPeriod, 'school' => $this->currentSchool]);
        if (!$classPeriods) {
            return new JsonResponse(['error' => 'Classe introuvable.'], 404);
        }
        $studentClass = $studentClassRepo->findOneBy(['student' => $studentId, 'schoolClassPeriod' => $classPeriods]);
