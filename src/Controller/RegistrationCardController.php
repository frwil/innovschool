...
class RegistrationCardController extends AbstractController
{
    ...
    public function index(SessionInterface $session): Response
    {
        ...
        if (!$config) {
            ...
            $schoolSections=array_map(function($s){ return ['id'=>$s->getId(),'name'=>$s->getName()];},$schoolSections);
        }
    ...
    public function getClassesBySection(SessionInterface $session,Request $request, SchoolSectionRepository $schoolSectionRepository, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        if (!$config) {
            ...
            $sections = array_map(function($s){ return $s->getId();},$sections);
        }
...
class RegistrationCardController extends AbstractController
{
    ...
    public function getClassesBySection(SessionInterface $session,Request $request, SchoolSectionRepository $schoolSectionRepository, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        if ($config) {
            $authorizedClasses = $config->getClassList();
        } else {
            $authorizedClasses = [];
            if (in_array('ROLE_SUPER_ADMIN',$this->getUser()->getRoles())) {
                $authorizedClasses = array_map(function ($class) {
    ...
    public function getStudentsByClass(SessionInterface $session,Request $request, UserRepository $userRepository): JsonResponse
    {
        ...
        if ($classId != null) {
            ...
            $schoolPeriod = $this->currentPeriod;
            $class = $this->schoolClassRepository->findBy(['classOccurence'=>$classId]);
            if (!$class) {
                return new JsonResponse(['error' => 'Class not found'], Response::HTTP_NOT_FOUND);
            }
            // Récupérer les élèves pour la classe donnée
    ...
    public function printAllCards(SessionInterface $session,Request $request): Response
    {
        ...
        foreach ($students as $student) {
            ...
            $studentInfos[$student->getId()] = $this->studentRepository->findOneBy(['student' => $student, 'period' => $this->currentPeriod]);

            // Générer le QR code
            $qrCode = new QrCode(
                "Matricule: {$student->getRegistrationNumber()}, Nom: {$student->getFullName()}, Classe: {$studentInfos[$student->getId()]->getSchoolClass()->getName()}",
                new Encoding('UTF-8'),
                ErrorCorrectionLevel::Low,
                70,
                0,
            ...
            // Validate the result
            $writer->validateResult($result, "Matricule: {$student->getRegistrationNumber()}, Nom: {$student->getFullName()}, Classe: {$studentInfos[$student->getId()]->getSchoolClass()->getName()}");
