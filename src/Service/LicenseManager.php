...
class LicenseManager
{
    ...
    {
        $startAt = $license->getLicenceStartAt();
        $data = ($startAt ? $startAt->format('Y-m-d H:i:s') : '')
    ...
    public function validateAndApplyToken(AppLicense $license, string $token): bool
    {
        // Exemple de validation simple : le token doit être égal à un hash attendu
        // À adapter selon ta logique réelle de génération de token

        // Exemple : le token doit être le hash généré pour une nouvelle période
        $now = new \DateTimeImmutable('now');
        $duration = 30; // ou extraire la durée du token si elle est encodée dedans

        // On simule ici que le token attendu est un hash de la date du jour + durée + secret
        $expectedToken = hash('sha256', $now->format('Y-m-d') . '|' . $duration . '|' .$license->getLicenseHash(). '|' . $this->secret);

        if ($token === $expectedToken) {
            // Applique la nouvelle licence
            $license->setLicenceStartAt(\DateTime::createFromImmutable($now));
            $license->setLicenceDuration($duration);
            $license->setLicenseHash($this->generateLicenseHash($license));
            return true;
        }
