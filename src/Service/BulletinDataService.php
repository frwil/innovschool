...
class BulletinDataService
{
    ...
    /**
     * Récupère tous les SchoolEvaluationTime liés à une classe, groupés par SchoolEvaluationFrame.
     * Utilisé pour le mode "Annuel" (periodicityId = 'all').
     * Les times sont triés par nom, les frames sont triées par ID.
     */
    public function getAllTimesGroupedByFrame(int $classId, BulletinContextService $context): array
    {
        $class = $this->getClass($classId);
        $school = $context->getCurrentSchool();
        $period = $context->getCurrentPeriod();
        ...
        foreach ($evaluations as $evaluation) {
            $time = $evaluation->getTime();
            if (!$time || in_array($time->getId(), $seenTimeIds)) {
                continue;
            }
            $seenTimeIds[] = $time->getId();
            
            $frame = $time->getEvaluationFrame();
            $frameId = $frame ? $frame->getId() : 0;
            
            if (!isset($framesMap[$frameId])) {
                $framesMap[$frameId] = [
                    'frame' => $frame,
                    'frame_name' => $frame ? $frame->getName() : 'Sans période',
                    'frame_short_name' => $frame ? $frame->getShortName() : 'SP',
                    'times' => [],
                ];
            }
            
            $framesMap[$frameId]['times'][] = $time;
        }

        // Trier les times par nom dans chaque frame
        foreach ($framesMap as &$frameData) {
            usort($frameData['times'], function ($a, $b) {
                return strcmp($a->getName() ?? '', $b->getName() ?? '');
            });
        }

        // Trier les frames par ID (croissant)
        ksort($framesMap);

        return array_values($framesMap);
    }
    ...
    public function calculateEvaluationProgress(ProgressQueryDTO $dto, BulletinContextService $context): array
    {
        ...
        $isAnnual = ($dto->periodicityId === 'all');
        ...
        if ($isAnnual) {
            ...
            $allEvaluations = $this->evaluationRepo->findBy(['classSubjectModule' => $modules]);
            $timeIds = [];
            foreach ($allEvaluations as $evaluation) {
                $timeId = $evaluation->getTime()->getId();
                if (!in_array($timeId, $timeIds)) {
                    $timeIds[] = $timeId;
                }
            }
            $allTimes = $this->timeRepo->findBy(['id' => $timeIds]);
        } elseif ($dto->bulletinType == 'sub-period') {
            $allTimes = [$this->timeRepo->find($dto->periodicityId)];
        } else {
            $frame = $this->frameRepo->find($dto->periodicityId);
            $allTimes = $this->timeRepo->findBy(['evaluationFrame' => $frame]);
        }
