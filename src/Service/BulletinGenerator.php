...
class BulletinGenerator
{
    ...
    public function generateBulletinA(
        ...
        if ($groupedFrames !== null) {
            // Mode Annuel : $periods = tous les times de tous les frames (aplati)
            $periods = [];
            ...
            $isAnnual = true;
        } elseif ($bulletinType == 'sub-period') {
        ...
        if ($reportCardTemplate == 'A') {
            ...
            $sheet->setCellValue('A3', ($isAnnual ? 'Annuel' : strtoupper(str_replace('è', 'e', str_replace('é', 'e', $periodicity[0]->getName())))) . ' - Année scolaire : ' . $currentPeriod->getName());
            ...
            if ($isAnnual && $groupedFrames) {
                // Ligne 8 : en-têtes des frames (fusionnées)
                $colOffset = 76; // Colonne L en ASCII
                $summaryColOffset = 71; // Colonne G en ASCII pour le résumé

                foreach ($groupedFrames as $frameData) {
                    $frameColspan = count($frameData['times']) * 3;
                    $startCol = chr($colOffset);
                    $endCol = chr($colOffset + $frameColspan - 3);

                    // Fusionner les cellules pour le nom du frame sur la ligne 8
                    if ($startCol !== $endCol) {
                        $sheet->mergeCells($startCol . '8:' . $endCol . '8');
                    }
                    $sheet->setCellValue($startCol . '8', $frameData['frame_name']);

                    // Style centré pour l'en-tête de frame
                    $sheet->getStyle($startCol . '8:' . $endCol . '8')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);

                    $colOffset += $frameColspan;
                }

                // Ligne 9 : en-têtes des times (sous-périodes) - Note + Barème fusionnés
                $colOffset = 76;
                foreach ($groupedFrames as $frameData) {
                    foreach ($frameData['times'] as $time) {
                        $sheet->getColumnDimension(chr($colOffset))->setVisible(true);
                        $sheet->getColumnDimension(chr($colOffset + 1))->setVisible(true);
                        $sheet->setCellValue(chr($colOffset) . '9', $time->getShortName());

                        // Fusionner Note + Barème pour l'en-tête de time
                        $sheet->mergeCells(chr($colOffset) . '9:' . chr($colOffset + 1) . '9');

                        $colOffset += 3;
                    }
                }

                // Résumé en bas (ligne 194+)
                $summaryColOffset = 71;
                foreach ($groupedFrames as $frameData) {
                    foreach ($frameData['times'] as $time) {
                        $sheet->setCellValue(chr($summaryColOffset) . '194', $time->getShortName());
                        $summaryColOffset++;
                    }
                }

                $k = $colOffset - 76;
                $l = $summaryColOffset - 71;
            } else {
                foreach ($periods as $periode) {
                    $tid = $periode->getId();
            ...
            $studentId = $student[0]->getStudent()->getId(); // l'ID de l'élève concerné
            $nbMatieres10 = 0;
            $nbMat = 0;
            ...
            $disciplines = $this->attendanceRepo->findBy(['studentClass' => $student[0]]);
    ...
    <thead>';
                if ($isAnnual && $groupedFrames) {
                    ...
                    $html .= '<th rowspan="2">Moy.</th><th rowspan="2">Coef.</th><th rowspan="2">Note x Coef.</th><th rowspan="2">Rang</th><th rowspan="2">Cotation</th><th rowspan="2">Grade</th><th rowspan="2">Min</th><th rowspan="2">Moy. Gen.</th><th rowspan="2">Max</th><th rowspan="2">% Réuss.</th><th rowspan="2">Signature enseignant</th></tr>';

                    // Ligne des times
                    $html .= '<tr class="bg-secondary">';
                    foreach ($groupedFrames as $frameData) {
                        foreach ($frameData['times'] as $time) {
                            $html .= '<th>' . $time->getShortName() . '</th>';
                        }
                    }
                    $html .= '</tr>';
                } else {
        ...
        <th>Matière</th>';
                ...
                $sum_coef = [];
                $total_moy = 0;
                //dd($sg->getSchoolClassSubjects()->toArray());
            ...
            if ($isAnnual && $groupedFrames) {
                // Ligne des frames
                $html .= '<tr class="bg-secondary">';
                $html .= '<th rowspan="3" style="vertical-align:middle;width:30%">Groupe de matières</th>';
                $html .= '<th rowspan="3" style="vertical-align:middle">Matières</th>';
                $html .= '<th rowspan="3" style="vertical-align:middle">Modules</th>';
                $html .= '<th></th>';
                foreach ($groupedFrames as $frameData) {
                    $colspan = count($frameData['times']) * ($evaluationColumns - 1);
                    $html .= '<th colspan="' . $colspan . '" style="text-align:center">' . $frameData['frame_name'] . '</th>';
                }
                $html .= '</tr>';
                ...
                foreach ($groupedFrames as $frameData) {
                    foreach ($frameData['times'] as $time) {
                        $html .= '<th colspan="' . ($evaluationColumns - 1) . '" style="text-align:center">' . $time->getShortName() . '</th>';
                    }
                }
                $html .= '</tr>';
    ...
    <div class="col-12 text-center" style="font-size:14pt">' . $reportCard->getHeaderTitle() . '</div>
            ...
            $html .= '<div class="row">
    <div class="col-12 text-center" style="font-size:9pt">' . $currentPeriod->getName() . ' - ' . ($isAnnual ? 'Annuel' : strtoupper($periodicity[0]->getName())) . '</div>
    </div>';
    ...
    <div class="col-6"><span style="font-weight:bold">' . ($bulLang === 'fr' ? 'Nom du tuteur : ' : 'Tutor Name: ') . '</span>' . ($student[0]->getStudent()->getTutor() !== null ? $student[0]->getStudent()->getTutor()->getFullName() . '/' . $student[0]->getStudent()->getTutor()->getPhone() : '') . '</div><div class="col-4"><span style="font-weight:bold">' . ($bulLang === 'fr' ? 'Enseignant principal : ' : 'Class Master: ') . '</span>' . ($student[0]->getSchoolClassPeriod()->getClassMaster() !== null ? $student[0]->getSchoolClassPeriod()->getClassMaster()->getFullName() : '') . '</div><div class="col-2"><img class="student-photo" src="' . $this->params->get('kernel.project_dir') .  '/public' . ($student[0]->getStudent()->getPhoto() ? '/uploads/' . $student[0]->getStudent()->getPhoto() : '/img/default_student.png') . '" width="60px" height="60px" style="position:relative"></div>
            ...
            if ($class->getSchoolClassSubjects()->count() > 0) {
                foreach ($subjectGroups as $subjectGroup) :
                    $html .= '<table class="table table-bordered" style="font-size:0.9em"><thead><tr><th colspan="' . $headerColspan . '"><h5 style="font-weight:bold;margin-top:0;margin-bottom:0;">' . $subjectGroup->getDescription() . '</h5></th></tr>';

                    if ($isAnnual && $groupedFrames) {
                        // Ligne des frames
                        $html .= '<tr class="coloured-header">';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Sujet' : 'Subject') . '</th>';
                        foreach ($groupedFrames as $frameData) {
                            $colspan = count($frameData['times']);
                            $html .= '<th colspan="' . $colspan . '" style="text-align:center">' . $frameData['frame_name'] . '</th>';
                        }
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Barème' : 'Grading sc.') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Moy.' : 'Avg.') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Rang' : 'Rank') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Cotation' : 'Rating') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Min.' : 'Min.') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Moy. gén.' : 'Gen. Avg.') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'Max.' : 'Max.') . '</th>';
                        $html .= '<th rowspan="2">' . ($bulLang === 'fr' ? 'T.R%' : 'S.R%') . '</th>';
                        $html .= '</tr>';
                        ...
                        $html .= '<tr class="coloured-header"><th>' . ($bulLang === 'fr' ? 'Sujet' : 'Subject') . '</th>';
                        foreach ($periods as $period) $html .= '<th>' . $period->getShortName() . '</th>';
                        $html .= '<th>' . ($bulLang === 'fr' ? 'Barème' : 'Grading sc.') . '</th><th>' . ($bulLang === 'fr' ? 'Moy.' : 'Avg.') . '</th><th>' . ($bulLang === 'fr' ? 'Rang' : 'Rank') . '</th><th>' . ($bulLang === 'fr' ? 'Cotation' : 'Rating') . '</th><th>' . ($bulLang === 'fr' ? 'Min.' : 'Min.') . '</th><th>' . ($bulLang === 'fr' ? 'Moy. gén.' : 'Gen. Avg.') . '</th><th>' . ($bulLang === 'fr' ? 'Max.' : 'Max.') . '</th><th>' . ($bulLang === 'fr' ? 'T.R%' : 'S.R%') . '</th></tr>';
                    }
                    ...
                    foreach ($subjectGroup->getSchoolClassSubjects()->toArray() as $subject) {
                        if ($subject->getSchoolClassPeriod()->getId() == $class->getId()) {
                            ...
                            foreach ($modules as $module) {
                                ...
                                $html .= '<td class="cell-number">' . $generalAverage . '</td>';
...
class BulletinGenerator
{
     ...
     */
    public function generateBulletin($studentId, $periodicityId, $bulletinType, $classId, $user)
    {
        ...
        $subjectGroups = $this->subjectGroupRepo->findBy(['class' => $class, 'period' => $period, 'school' => $school]);
        $subjectGroupsIds = array_map(function ($subjectGroup) {
            return $subjectGroup->getId();
        }, $subjectGroups);

        $subjects = $this->schoolClassSubjectRepo->findBy(['group' => $subjectGroupsIds, 'period' => $period, 'school' => $school], ['group' => 'ASC']);
        if (!$subjects) {
            throw new NotFoundHttpException('Aucun sujet trouvé pour cette classe.');
        }
        ...
        $filePath = $this->params->get('kernel.project_dir') . '/public/uploads/bulletins/bulletin_test.xlsx';
        $spreadsheet = IOFactory::load($filePath);

        $sheet = $spreadsheet->getSheetByName('Stats');
        $sheet->setSheetState(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_HIDDEN);
        /* $sheet->removeRow(2, 50000); // Supprime les lignes 2 à 10

        $i = 1;
        $sheet->setCellvalue('A1', 'Id_evaluation');
        $sheet->setCellValue('B1', 'Student');
        $sheet->setCellValue('C1', 'class_module_id');
        $sheet->setCellValue('D1', 'Module_id');
        $sheet->setCellValue('E1', 'Module_name');
        $sheet->setCellValue('F1', 'class_module_period');
        $sheet->setCellValue('G1', 'class_module_classe');
        $sheet->setCellValue('H1', 'class_module_school');
        $sheet->setCellValue('I1', 'module_notation');
        $sheet->setCellValue('J1', 'evaluation_note');
        $sheet->setCellValue('K1', 'evaluation_time_id');
        $sheet->setCellValue('L1', 'evaluation_time_name');
        $sheet->setCellValue('M1', 'Student_id');
        $sheet->setCellValue('N1', 'Student_class_id');
        $sheet->setCellValue('O1', 'Student_class_name');
        $sheet->setCellValue('P1', 'Student_class_period_id');
        $sheet->setCellValue('Q1', 'Student_class_period_name');
        $sheet->setCellValue('R1', 'Student_registration_number');


        foreach ($evaluations as $evaluation) {
            $sheet->setCellValue('A' . ($i + 1), $evaluation->getId());
            $sheet->setCellValue('B' . ($i + 1), $evaluation->getStudent()->getStudent()->getFullName());
            $sheet->setCellValue('C' . ($i + 1), $evaluation->getClassSubjectModule()->getId());
            $sheet->setCellValue('D' . ($i + 1), $evaluation->getClassSubjectModule()->getModule()->getId());
            $sheet->setCellValue('E' . ($i + 1), $evaluation->getClassSubjectModule()->getModule()->getModuleName());
            $sheet->setCellValue('F' . ($i + 1), $evaluation->getClassSubjectModule()->getPeriod()->getName());
            $sheet->setCellValue('G' . ($i + 1), $evaluation->getClassSubjectModule()->getClass()->getName());
            $sheet->setCellValue('H' . ($i + 1), $evaluation->getClassSubjectModule()->getSchool()->getName());
            $sheet->setCellValue('I' . ($i + 1), $evaluation->getClassSubjectModule()->getModuleNotation());
            $sheet->setCellValue('J' . ($i + 1), $evaluation->getEvaluationNote());
            $sheet->setCellValue('K' . ($i + 1), $evaluation->getTime()->getId());
            $sheet->setCellValue('L' . ($i + 1), $evaluation->getTime()->getShortName());
            $sheet->setCellValue('M' . ($i + 1), $evaluation->getStudent()->getStudent()->getId());
            $sheet->setCellValue('N' . ($i + 1), $evaluation->getStudent()->getSchoolClass()->getId());
            $sheet->setCellValue('O' . ($i + 1), $evaluation->getStudent()->getSchoolClass()->getName());
            $sheet->setCellValue('P' . ($i + 1), $evaluation->getStudent()->getPeriod()->getId());
            $sheet->setCellValue('Q' . ($i + 1), $evaluation->getStudent()->getPeriod()->getName());
            $sheet->setCellValue('R' . ($i + 1), $evaluation->getStudent()->getStudent()->getRegistrationNumber());
            $i++;
        }
        ...
        $c = 1;
        foreach ($periods as $periode) {
            $sheet->setCellValue('Z' . $c, $periode->getShortName());
            $sheet->setCellValue('AA' . $c, $periode->getName());
            $c++;
        }
        $i = 5;
        $sheet->removeRow(5, 5000); // Supprime les lignes 2 à 10
        foreach ($students as $eleve) {
            $sheet->setCellValue('A' . $i, $eleve->getStudent()->getRegistrationNumber());
            $sheet->setCellValue('B' . $i, $eleve->getStudent()->getFullName());
            $j = 0;
            foreach ($periods as $periode) {
                $sheet->setCellValue(chr(67 + $j) . $i, '=SUMIFS(Stats!$J:$J,Stats!$B:$B,$B' . $i . ',Stats!$L:$L,' . chr(67 + $j) . '$1,Stats!$F:$F,' . chr(67 + $j) . '$4,Stats!$G:$G,' . chr(67 + $j) . '$3,Stats!$H:$H,' . chr(67 + $j) . '$2)');
                $sheet->setCellValue(chr(71 + $j) . $i, '=SUMIFS(Stats!$I:$I,Stats!$B:$B,$B' . $i . ',Stats!$L:$L,' . chr(71 + $j) . '$1,Stats!$F:$F,' . chr(71 + $j) . '$4,Stats!$G:$G,' . chr(71 + $j) . '$3,Stats!$H:$H,' . chr(71 + $j) . '$2)');
                $sheet->setCellValue(chr(75 + $j) . $i, '=IFERROR(' . chr(67 + $j) . '' . $i . '/' . chr(71 + $j) . '' . $i . '*20,0)');
                $sheet->setCellValue(chr(78 + $j) . $i, '=RANK.EQ(' . chr(75 + $j) . '' . $i . ',$' . chr(75 + $j) . '$5:$' . chr(75 + $j) . '$5000,0)');
                $j++;
            }
            $sheet->setCellValue('Q' . $i, '=IFERROR(SUM(C' . $i . ':E' . $i . ')/SUM(G' . $i . ':I' . $i . ')*20,0)');
            $sheet->setCellValue('R' . $i, '=RANK.EQ(' . chr(79 + $j) . '' . $i . ',$' . chr(79 + $j) . '$5:$' . chr(79 + $j) . '$5000,0)');
            $sheet->setCellValue('S' . $i, '=IFERROR(AVERAGEIF(C' . $i . ':E' . $i . ',">0"),0)');
            $sheet->setCellValue('T' . $i, '=IFERROR(AVERAGEIF(G' . $i . ':I' . $i . ',">0"),0)');
        ...
        $i = 0;
        foreach ($periods as $periode) {
            $sheet->setCellValue(chr(67 + $i) . '1', $periode->getShortName());
            $sheet->setCellValue(chr(71 + $i) . '1', $periode->getShortName());
            $sheet->setCellValue(chr(75 + $i) . '1', $periode->getShortName());
            $sheet->setCellValue(chr(67 + $i) . '2', $school->getName());
            $sheet->setCellValue(chr(71 + $i) . '2', $school->getName());
            $sheet->setCellValue(chr(75 + $i) . '2', $school->getName());
            $sheet->setCellValue(chr(67 + $i) . '3', $class->getName());
            $sheet->setCellValue(chr(71 + $i) . '3', $class->getName());
            $sheet->setCellValue(chr(75 + $i) . '3', $class->getName());
            $sheet->setCellValue(chr(67 + $i) . '4', $period->getName());
            $sheet->setCellValue(chr(71 + $i) . '4', $period->getName());
            $sheet->setCellValue(chr(75 + $i) . '4', $period->getName());
        ...
        // $sheet->mergeCells('A1:B1', true);
        // $sheet->mergeCells('D1:' . chr(68 + count($periods) * 2 - 1) . '1', true);
        $sheet->setCellValue('A1', $school->getName() . chr(10) . $school->getAddress() . chr(10) . $school->getContactPhone() . chr(10) . $school->getContactEmail());
        // // Insérer le logo dans la cellule D1
        $logo1 = new Drawing();
        $logo1->setName('Logo_ecole');
        $logo1->setDescription('Logo de l\'école');
        $logo1->setPath($this->params->get('kernel.project_dir') . '/public/img/logo_test.png'); // Chemin vers le fichier du logo
        $logo1->setHeight(80); // Hauteur du logo en pixels
        $logo1->setCoordinates('G1'); // Positionner le logo dans la cellule D1
        $logo1->setWorksheet($sheet);

        //mettre la photo de l'élève 

        $logo = new Drawing();
        $logo->setName('photo_eleve');
        $logo->setDescription('Photo de l\'élève');
        $logo->setPath($this->params->get('kernel.project_dir') . '/public/uploads/' . $student[0]->getStudent()->getPhoto()); // Chemin vers le fichier du logo
        $logo->setHeight(80); // Hauteur du logo en pixels
        $logo->setCoordinates('I5'); // Positionner le logo dans la cellule D1
        $logo->setWorksheet($sheet);

        // Centrer horizontalement et verticalement la cellule G1
        $sheet->getStyle('G1')->getAlignment()
            ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT)
            ->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);


        $sheet->setCellValue('D1', $school->getName() . chr(10) . $school->getAddress() . chr(10) . $school->getContactPhone() . chr(10) . $school->getContactEmail());

        $sheet->setCellValue('A3', strtoupper(str_replace('è', 'e', str_replace('é', 'e', $periodicity[0]->getName()))) . ' - Année scolaire : ' . $period->getName());
        $sheet->setCellValue('C4', strtoupper($student[0]->getStudent()->getFullName()));
        $sheet->setCellValue('M4', $student[0]->getStudent()->getRegistrationNumber());
        if ($student[0]->getStudent()->getDateOfBirth()) {
            $sheet->setCellValue('C5', date_format($student[0]->getStudent()->getDateOfBirth(), 'd/m/Y'));
        ...
        $sheet->setCellValue('C6', $student[0]->getStudent()->getGender() == 'M' ? 'Masculin' : 'Féminin');
        $sheet->setCellValue('C7', $class->getName());
        ...
        $lastCol = $sheet->getCell(chr(76 + $k - 2) . '9')->getColumn();
        if ($lastCol != 'S') {
            $sheet->getColumnDimension(chr(ord($lastCol) + 1))->setVisible(false);
            $sheet->getColumnDimension(chr(ord($lastCol) + 4))->setVisible(false);
            $sheet->getColumnDimension(chr(ord($lastCol) + 7))->setVisible(false);
        }
        $sheet->mergeCells('J1:' . $lastCol . '1', true);
        $sheet->getStyle('J1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A2:' . $lastCol . '2', true);
        $sheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A3:' . $lastCol . '3', true);
        $sheet->getStyle('A3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A202:' . $lastCol . '202', true);
        $sheet->getStyle('A202')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('A205:' . $lastCol . '205', true);
        $sheet->getStyle('A205')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('J207:' . $lastCol . '207', true);
        $sheet->getStyle('J207')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->mergeCells('L193:' . $lastCol . '193', true);
        $sheet->getStyle('L193')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->setCellValue('L209', $user->getFullName());
        $i = 11;
        $groupes = [];
        foreach ($subjects as $subject) {
            if (!isset($groupes[$subject->getGroup()->getId()])) {
                $groupes[$subject->getGroup()->getId()] = ['count' => 0, 'countSubjects' => 0];
            }
            $groupes[$subject->getGroup()->getId()]['countSubjects']++;
            $sheet->setCellValue('F' . $i, $subject->getName());
            $modules = $this->subjectModuleRepo->findBy(['subject' => $subject, 'period' => $period, 'school' => $school, 'class' => $class]);
            $j = 0;
            foreach ($modules as $module) {
                $groupes[$subject->getGroup()->getId()]['count']++;
                $sheet->setCellValue('I' . $j + $i, $module->getModule()->getModuleName() . ': ' . $module->getModuleNotation() . ' pts');
                $sheet->setCellValue('K' . $j + $i, $module->getModuleNotation());
                $sheet->getRowDimension($j + $i)->setVisible(true);
                $k = 0;
                foreach ($periods as $periode) {
                    $eval = $this->evaluationRepo->findBy([
                        'student' => $student[0],
                        'classSubjectModule' => $module,
                        'time' => $periode,
                    ]);
                    if ($eval) {
                        $sheet->setCellValue(chr(75 + $k) . ($j + $i), $module->getModuleNotation());
                        $sheet->setCellValue(chr(76 + $k) . ($j + $i), $eval[0]->getEvaluationNote() ?? '');
                    } else {
                        $sheet->setCellValue(chr(76 + $k) . ($j + $i), '');
                    }
                    $k += 3;
                }
                $j++;
            }
            $sheet->getRowDimension($i + 4)->setVisible(true);
            $col = 0;
            foreach ($periods as $periode) {
                $note = $sheet->getCell(chr(77 + $col) . ($i + 4))->getValue();
                if ($template) {
                    $appreciation = $this->appreciationService->getAppreciationByNote($template, $note);
                } else {
                    $appreciation = '';
                }
                $sheet->setCellValue(chr(77 + $col) . ($i + 4), $appreciation);
                $col += 3;
            }
            $note = $sheet->getCell('L' . ($i + 5))->getValue();
            $sheet->mergeCells('L' . ($i + 5) . ':' . $lastCol . ($i + 5), true);
            $sheet->getStyle('L' . ($i + 5))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('F' . ($i + 4) . ':J' . ($i + 5))->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFD9D9D9');
            $sheet->getStyle('F' . ($i + 5) . ':L' . ($i + 5))->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFD9D9D9');
            if ($template) {
                $appreciation = $this->appreciationService->getAppreciationByNote($template, $note);
            } else {
                $appreciation = '';
            }
            $sheet->setCellValue('L' . ($i + 5), $appreciation);
            $sheet->getRowDimension($i + 5)->setVisible(true);
            $i += 6;
        }
        $i = 11;
        foreach ($groupes as $key => $value) {
            $sheet->insertNewRowBefore($i + $value['count'] + $value['countSubjects'] * 2, 2);
            $sheet->mergeCells('L' . ($i + $value['count'] + $value['countSubjects'] * 2) . ':' . $lastCol . ($i + $value['count'] + $value['countSubjects'] * 2), true);
            $sheet->getStyle('L' . ($i + $value['count'] + $value['countSubjects'] * 2))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue('A' . ($i + $value['count'] + $value['countSubjects'] * 2), 'COTE');
            $sheet->getStyle('A' . ($i + $value['count'] + $value['countSubjects'] * 2) . ':' . $lastCol . ($i + $value['count'] + $value['countSubjects'] * 2))->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFD9D9D9');
            $sheet->setCellValue('F' . ($i + $value['count'] + $value['countSubjects'] * 2), '=SUMIFS(L' . $i . ':L' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',F' . $i . ':F' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',"Total")+SUMIFS(O' . $i . ':O' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',F' . $i . ':F' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',"Total")+SUMIFS(R' . $i . ':R' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',F' . $i . ':F' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',"Total")');
            $sheet->setCellValue('J' . ($i + $value['count'] + $value['countSubjects'] * 2), '=SUMIFS(K' . $i . ':K' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',F' . $i . ':F' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',"Total")+SUMIFS(N' . $i . ':N' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',F' . $i . ':F' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',"Total")+SUMIFS(Q' . $i . ':Q' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',F' . $i . ':F' . ($i + $value['count'] + $value['countSubjects'] * 2 - 2) . ',"Total")');
            $note = $sheet->getCell('F' . ($i + $value['count'] + $value['countSubjects'] * 2))->getCalculatedValue() * 20 / $sheet->getCell('J' . ($i + $value['count'] + $value['countSubjects'] * 2))->getCalculatedValue();
            if ($template) {
                $appreciation = $this->appreciationService->getAppreciationByNote($template, $note);
            } else {
                $appreciation = '';
            }
            $sheet->setCellValue('L' . ($i + $value['count'] + $value['countSubjects'] * 2), $appreciation);
            $sheet->mergeCells('A' . ($i + $value['count'] + $value['countSubjects'] * 2 + 1) . ':' . $lastCol . ($i + $value['count'] + $value['countSubjects'] * 2 + 1));
            $sheet->getStyle('A' . ($i + $value['count'] + $value['countSubjects'] * 2 + 1))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            if ($value > 1) {
                $sheet->mergeCells('A' . ($i) . ':E' . ($i + $value['count'] + $value['countSubjects'] * 2 - 1));
                $sheet->setCellValue('A' . ($i), $this->subjectGroupRepo->find($key)->getDescription());
            }
            $sheet->getStyle('A' . $i . ':E' . ($i + $value['count'] + $value['countSubjects'] * 2 - 1))
                ->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                            'borderStyle' => Border::BORDER_THIN,
            ...
            $sheet->getStyle('A' . ($i + $value['count'] + $value['countSubjects'] * 2) . ':E' . ($i + $value['count'] + $value['countSubjects'] * 2))
                ->applyFromArray([
                    'borders' => [
                        'top' => [
                            'borderStyle' => Border::BORDER_THIN,
            ...
            $i += $value['count'] + $value['countSubjects'] * 2 + 2;
        }

        $c = 196;
        while ($sheet->getCell('F' . $c)->getValue() != 'MOYENNE') {
            $c++;
        }
        $cl = 0;
        foreach ($periods as $periode) {
            $cellCoordinate = $sheet->getCell(chr(71 + $cl) . $c)->getColumn() . $c;
            if ($cellCoordinate !== null) {
                $note = $sheet->getCell($cellCoordinate)->getValue();
                if ($template) {
                    $appreciation = $this->appreciationService->getAppreciationByNote($template, $note);
                } else {
                    $appreciation = '';
                }
                $row = $sheet->getCell($cellCoordinate)->getRow() + 1;
                $col = $sheet->getCell($cellCoordinate)->getColumn();
                $sheet->setCellValue($col . $row, $appreciation);
            }
            $cl++;
        }
        $note = $sheet->getCell('J' . $c)->getValue();
        if ($template) {
            $appreciation = $this->appreciationService->getAppreciationByNote($template, $note);
        } else {
            $appreciation = '';
        }
        $sheet->setCellValue('J' . $c + 1, $appreciation);

        $c -= 2;
        $row = $c;
        for ($cl = 0; $cl < 3; $cl++) {
            $sheet->mergeCells('M' . $row + $cl . ':' . $lastCol . $row + $cl, true);
            $sheet->getStyle('M' . $row + $cl)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        }
        if ($template) {
            $appreciationBaremes = $this->appreciationBaremeRepo->findBy(['evaluationAppreciationTemplate' => $template]);
            $maxnote = 0;
            foreach ($appreciationBaremes as $appreciationBareme) {
                $sheet->setCellValue('A' . $c, $appreciationBareme->getEvaluationAppreciationValue());
                $sheet->setCellValue('B' . $c, $appreciationBareme->getEvaluationAppreciationFullValue());
                $sheet->setCellValue('C' . $c, $maxnote . '-' . $appreciationBareme->getEvaluationAppreciationMaxNote());
                $maxnote = $appreciationBareme->getEvaluationAppreciationMaxNote();
                $c++;
            }
        ...
        $sheet->getColumnDimension('A')->setWidth(20); // Largeur approximative pour 100px
        $sheet->getRowDimension('1')->setRowHeight(80); // Hauteur de la ligne pour le logo
        $sheet->getRowDimension('2')->setRowHeight(40);
        $sheet->getRowDimension('3')->setRowHeight(40);
        // Ajouter des bordures de la ligne 9 à la dernière ligne ($i)
        $sheet->getStyle('A9:' . $sheet->getHighestColumn() . '10')
            ->applyFromArray([
                'borders' => [
                    'allBorders' => [
                        'borderStyle' => Border::BORDER_THIN,
        ...
        // $sheet->getPageSetup()
        //     ->setPrintArea('A1:' . $sheet->getHighestColumn() . $i); // Définir la zone d'impression (de A1 à la dernière cellule)

        // Configurer le type de papier
        // $sheet->getPageSetup()
        //     ->setPaperSize(PageSetup::PAPERSIZE_A4); // Format A4

        // // Configurer l'orientation de la page
        // $sheet->getPageSetup()
        //     ->setOrientation(PageSetup::ORIENTATION_PORTRAIT); // Portrait (ou utiliser ORIENTATION_LANDSCAPE pour paysage)

        // // Ajuster le contenu à la page
        // $sheet->getPageSetup()
        //     ->setFitToWidth(1) // Ajuster à 1 page en largeur
        //     ->setFitToHeight(0); // Ajuster automatiquement la hauteur

        // // Configurer les marges de la page
        // $sheet->getPageMargins()->setTop(0.5); // Marge supérieure (en pouces)
        // $sheet->getPageMargins()->setRight(0.5); // Marge droite
        // $sheet->getPageMargins()->setLeft(0.5); // Marge gauche
        // $sheet->getPageMargins()->setBottom(0.5); // Marge inférieure
        ...
        // // Nom du fichier HTML
        // $fileName = $fileName = $student[0]->getStudent()->getRegistrationNumber() . '.html';
        // $filePath = $this->params->get('kernel.project_dir') . '/public/uploads/bulletins/' . $fileName;

        // // Convertir la feuille en HTML
        // $htmlWriter = new Html($spreadsheet);
        // // Enregistrer le fichier HTML
        // $htmlWriter->save($filePath);

        // $html = file_get_contents($filePath);
        // $html = str_replace('src="file:///' . str_replace('\\', '/', $this->params->get('kernel.project_dir') . '/public'), 'src="', $html);
        // $html = str_replace('<title>Untitled Spreadsheet</title>', '<title>Bulletin ' . $student[0]->getStudent()->getRegistrationNumber() . '</title>', $html);
        // $html = str_replace('<body>', '<body style="margin: 0; padding: 0;">', $html);
        // $html = str_replace('</body>', '<script>setTimeout(()=>{try{document.body.lastElementChild.style.display="none";window.print();window.onafterprint = function() {self.close();};}catch(e){window.print()}},1000);</script><style>@page {margin: 10mm;@bottom-center {content: "Page " counter(page) " / " counter(pages);font-size: 12px;color: #000;}}@media print {html, body {
        // margin: 0 !important;
        // padding: 0 !important;
    ...
    public function generateBulletinA($studentId, $periodicityId, $bulletinType, $classId, $user, $currentSchool, $currentPeriod, $reportCardTemplate = 'A')
    {
        $student = $this->studentRepo->findByStudent($studentId);
        if (!$student) {
            throw new NotFoundHttpException('Élève non trouvé.');
        ...
        $class = $this->classRepo->findOneBy(['classOccurence' => $classId, 'period' => $currentPeriod, 'school' => $school]);
        if (!$class) {
            throw new NotFoundHttpException('Classe non trouvée.');
        }
        ...
        $subjectGroupsIds = array_map(fn($subjectGroup) => $subjectGroup->getId(), $subjectGroups);
        ...
        if ($reportCardTemplate == 'A') {
            ...
            // $spreadsheet->setCalculationEngine(null); // Ligne supprimée car la méthode n'existe pas

            // foreach (range(2, 5000) as $row) {
            //     foreach (range('A', 'R') as $col) {
            //         $sheet->setCellValue($col . $row, '');
            //     }
            // }
            // $sheet->removeRow(2, $sheet->getHighestRow()); // Supprime les lignes 2 à 10
            ...
            // $arrData = $sheet->rangeToArray('A5:T' . $sheet->getHighestRow(), NULL, true, FALSE);
            ...
            foreach ($subjectGroups as $sg) {
                $subjectsFiltered = array_filter($subjectsClass, function ($s) use ($sg) {
                    return $s->getGroup()->getId() == $sg->getId();
                });
                $k = 0;
                foreach ($periods as $periode) {
                    $tid = $periode->getId();
                ...
                $sheet->setCellValue('A' . ($i + (int)(($j + $k) / 2) - 1), $sg->getDescription());
                $sheet->mergeCells('A' . ($i + (int)(($j + $k) / 2) - 1) . ':E' . ($i + (int)(($j + $k) / 2) - 1), true);
                $styleBorders = [
                    'borders' => [
                        'top' => [
                            'borderStyle' => Border::BORDER_THIN,
            ...
            //     $groupName = $subject->getGroup()->getDescription();

            //     if (!isset($groupes[$groupId])) {
            //         $groupes[$groupId] = ['count' => 0, 'countSubjects' => 0, 'groupName' => $groupName];
            //     }
            //     $groupes[$groupId]['countSubjects']++;
            //     $sheet->setCellValue('F' . $i, $subject->getName());

            //     // Utilise les modules préchargés, pas de requête :
            //     $subjectModules = $modulesBySubject[$subjectId] ?? [];
            //     $j = 0;
            //     foreach ($subjectModules as $module) {
            //         $sheet->setCellValue('I' . ($j + $i), $module->getModule()->getModuleName() . ': ' . $module->getModuleNotation() . ' pts');
            //         $sheet->setCellValue('K' . ($j + $i), $module->getModuleNotation());
            ...
            //             // Utilise l'indexation pour récupérer l'évaluation
            //             $eval = $evalIndex[$studentId][$module->getId()][$periodeId] ?? null;
            //             if ($eval) {
            //                 $sheet->setCellValue(chr(75 + $k) . ($j + $i), $module->getModuleNotation());
            //                 $sheet->setCellValue(chr(76 + $k) . ($j + $i), $eval->getEvaluationNote() ?? '');
            //             } else {
            //                 //$sheet->setCellValue(chr(76 + $k) . ($j + $i), '');
            //             }
            //             $k += 3;
            ...
            //     // Appréciations par matière (exemple)
            //     $col = 0;
            //     $sheet->getRowDimension($i + 4)->setVisible(true);
            //     $sheet->getRowDimension($i + 5)->setVisible(true);
            //     foreach ($periods as $periode) {
            //         $noteCell = chr(77 + $col) . ($i + 4); // adapte selon ton modèle
            //         $note = $sheet->getCell($noteCell)->getCalculatedValue();
            //         $appreciation = $template ? $this->appreciationService->getAppreciationByNote($template, $note) : '';
            //         $sheet->setCellValue($noteCell, $appreciation);
            //         $col += 3;
            //     }
            //     $note = $sheet->getCell('L' . ($i + 5))->getCalculatedValue();
            //     $appreciation = $template ? $this->appreciationService->getAppreciationByNote($template, $note) : '';
            //     $sheet->setCellValue('L' . ($i + 5), $appreciation);

            //     $i += 6; // adapte selon ta logique de lignes
            // }

            // // Totaux et appréciations par groupe
            // $i = 11;
            // foreach ($groupes as $groupId => $value) {
            //     $sheet->setCellValue('A' . (int)(($value['count'] + $value['countSubjects'] * 2) / 2) + $i, $value['groupName']);
            //     $row = $i + $value['count'] + $value['countSubjects'] * 2;
            //     $sheet->insertNewRowBefore($row, 2);
            //     //$sheet->mergeCells('L' . $row . ':S' . $row, true);
            //     //$sheet->getStyle('L' . $row)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            //     $sheet->setCellValue('A' . $row, 'COTE');
            //     // Calculs de totaux (exemple, adapte selon ton modèle)
            //     $note = $sheet->getCell('F' . $row)->getCalculatedValue() * 20;  // $sheet->getCell('J' . $row)->getCalculatedValue()!=0 ? $sheet->getCell('J' . $row)->getCalculatedValue() : 1;
            //     $appreciation = $template ? $this->appreciationService->getAppreciationByNote($template, $note) : '';
            //     $sheet->setCellValue('L' . $row, $appreciation);
            //     $i += $value['count'] + $value['countSubjects'] * 2 + 2;
            // }

            // Appréciation générale (exemple)
            // $note = $sheet->getCell('J20')->getValue(); // adapte la cellule
            // $appreciation = $template ? $this->appreciationService->getAppreciationByNote($template, $note) : '';
            // $sheet->setCellValue('J21', $appreciation); // adapte la cellule
            ...
            // $fileName = $student[0]->getStudent()->getRegistrationNumber() . '.xlsx';
            // $filePath = $this->params->get('kernel.project_dir') . '/public/uploads/bulletins/' . $fileName;
            // $writer = new Xlsx($spreadsheet);
            // $writer->setPreCalculateFormulas(false); // Désactiver le calcul automatique
            // $writer->save($filePath);
            ...
            // $html = str_replace('<title>Untitled Spreadsheet</title>', '<title>Bulletin ' . $student[0]->getStudent()->getRegistrationNumber() . '</title>', $html);
            // file_put_contents($htmlFilePath, $html);
            return [$htmlFilePath, $lastLigne, $lignesEntete, ord($lastCol), $sheet->getHighestRow()];
        } elseif ($reportCardTemplate == 'B') {
            // Code pour le modèle B
            // ...
            $subjects = [];
            ...
            foreach ($class->getSchoolClassSubjects() as $subject) {
                ...
                foreach ($class->getStudentClasses() as $studentClass) {
                    ...
                    foreach ($periods as $periode) {
                        ...
                        $modules = $subject->getStudySubject()->getClassSubjectModules()->toArray();
                        $modules = array_filter($modules, function ($m) {
                            return $m->getModule()->getModuleName() == 'Standard/Ecrit';
                        });
                        $modules = array_values($modules);
                        $mid = isset($modules[0]) ? $modules[0]->getId() : null;
                        $eval = $evalIndex[$studentId][$subjectId][$mid][$tid] ?? null;
            ...
            <div class="col-md-12 text-center" style="font-size:20pt;font-weight:bold">BULLETIN DE NOTES</div>
            <div class="col-md-12 text-center">' . $currentPeriod->getName() . '</div>
            <div class="col-md-12 text-center">' . strtoupper($periodicity[0]->getName()) . '</div>
            <div class="row">
                <div class="col-md-9">
                    <div class="row" style="font-size:10pt">
                        <div class="col-md-5">Nom : <strong>' . strtoupper($student[0]->getStudent()->getFullName()) . '</strong></div><div class="col-md-4">Classe : <strong>' . $class->getClassOccurence()->getName() . '</strong></div><div class="col-md-3">Redoublant : <strong></strong></div>
            ...
            foreach ($subjectGroups as $sg) {
                ...
                $sum_coef = [];
                $total_moy = 0;
                foreach ($sg->getSchoolClassSubjects() as $subject) {
                    $html .= '<tr><td style="font-weight:bold">' . $subject->getStudySubject()->getName() . ($subject->getTeacher() ? '<br><span style="font-size:0.7em;font-weight:lighter !important;font-style:italic">' . $subject->getTeacher()->getFullName() . '</span>' : '') . '</td>';
                    $total = 0;
                    foreach ($periods as $periode) {
                        $tid = $periode->getId();
                        $module = $subject->getStudySubject()->getClassSubjectModules()->toArray();
                        ...
                        $eval = $evalIndex[$student[0]->getStudent()->getId()][$subject->getStudySubject()->getId()][$mid][$tid] ?? null;
                        if ($eval) {
                            $html .= '<td class="text-end">' . $this->setNoteStyle(number_format($eval->getEvaluationNote(), 2), true) . '</td>';
                            $total += $eval->getEvaluationNote();
                            if (!isset($notes_coef[$tid])) $notes_coef[$tid] = 0;
                            if (!isset($sum_coef[$tid])) $sum_coef[$tid] = 0;
                            $notes_coef[$tid] += $eval->getEvaluationNote() * $subject->getCoefficient();
                    ...
                    $moy = $total / count($periods);
                    $nbMatieres10 += $moy >= 10 ? 1 : 0;
                    $nbMat++;
                    $total_moy += $moy * $subject->getCoefficient();
                    $nb_reussite = isset($notesMatieres[$subject->getStudySubject()->getId()]) ? count(array_filter($notesMatieres[$subject->getStudySubject()->getId()], function ($note) {
                        return $note >= 10;
                    })) : 0;
                    $taux_reussite = isset($notesMatieres[$subject->getStudySubject()->getId()]) ? $nb_reussite / count($notesMatieres[$subject->getStudySubject()->getId()]) * 100 : 0;
                    $notesMatieresCleaned = isset($notesMatieres[$subject->getStudySubject()->getId()]) ? array_filter($notesMatieres[$subject->getStudySubject()->getId()], function ($note) {
                    ...
                    $moyenne_generale = isset($notesMatieres[$subject->getStudySubject()->getId()]) ? array_sum($notesMatieres[$subject->getStudySubject()->getId()]) / count($notesMatieres[$subject->getStudySubject()->getId()]) : 0;
                    $html .= '<td class="text-end">' . $this->setNoteStyle(number_format($moy, 2), true) . '</td><td class="text-end">' . number_format($subject->getCoefficient(), 2) . '</td><td  class="text-end">' . number_format($moy * $subject->getCoefficient(), 2) . '</td><td class="text-end">' . (isset($notesMatieres[$subject->getStudySubject()->getId()]) ? $this->getRank($moy, $notesMatieres[$subject->getStudySubject()->getId()]) : '') . '</td><td class="text-center">' . ($this->getBareme($moy, $baremes) ? explode('(', $this->getBareme($moy, $baremes)->getEvaluationAppreciationFullValue())[0] : '') . '</td><td class="text-center">' . ($this->getBareme($moy, $baremes) ? $this->getBareme($moy, $baremes)->getEvaluationAppreciationValue() : '') . '</td><td class="text-end">' . (count($notesMatieresCleaned) > 0 ? min($notesMatieresCleaned) : '') . '</td><td class="text-end">' . number_format($moyenne_generale, 2) . '</td><td class="text-end">' . (count($notesMatieresCleaned) > 0 ? max($notesMatieresCleaned) : '') . '</td><td class="text-end">' . number_format($taux_reussite, 2) . '%</td><td></td></tr>'; // Appréciation à ajouter
                }
                $html .= '<tr class="bg-light" style="font-weight:bold;font-style:italic"><td>Récap</td>';
                foreach ($periods as $periode) {
                    $tid = $periode->getId();
            ...
            <div class="col-md-9">
                <div class="row table-content">
                    ...
                    <div class="col-md-3 text-end">' . $moyennesEleves[$student[0]->getStudent()->getId()] . '</div>
                    <div class="col-md-9 text-start">Rang</div>
                    <div class="col-md-3 text-end">' . $this->getRank($moyennesEleves[$student[0]->getStudent()->getId()], $moyennesEleves) . '</div>
                    <div class="col-md-9 text-start">Nombre de matières validées</div>
                    <div class="col-md-3 text-center">' . $nbMatieres10 . '/' . $nbMat . '</div>
                </div>
            </div>
            ...
            <div class="col-md-9">
                <div class="row table-content">
                    <div class="col-md-12 text-start text-danger" style="font-style:italic">* La moyenne générale est calculée sur 20</div>
                    <div class="col-md-9 text-start">Moyenne la plus faible</div>
                    <div class="col-md-3 text-end">' . min(array_filter($moyennesEleves, function ($moy) {
                return $moy != null;
            })) . '</div>
                    <div class="col-md-9 text-start">Moyenne la plus forte</div>
                    <div class="col-md-3 text-end">' . max($moyennesEleves) . '</div>
                    <div class="col-md-9 text-start">Taux de réussite</div>
                    <div class="col-md-3 text-end">' . number_format(count(array_filter($moyennesEleves, function ($moy) {
                return $moy >= 10;
            })) / count($moyennesEleves) * 100, 2) . '%</div>
                    <div class="col-md-9 text-start">Ecart-type</div>
                    <div class="col-md-3 text-end">' . number_format($this->getEcartType($moyennesEleves), 2) . '</div>
                    <div class="col-md-9 text-start">Moyenne Générale</div>
                    <div class="col-md-3 text-end">' . number_format($this->getMoyenneGenerale($moyennes), 2) . '</div>
                </div>
            </div>
            </div>
            </div>
     ...
     * @return int|null         Le rang (1 = meilleur), ou null si la note n'est pas trouvée
     */
    function getRank($note, array $notes, $desc = true): ?int
    {
        // Nettoie et trie le tableau
        $notes = array_filter($notes, fn($n) => $n !== null && $n !== '');
        $notes = array_map('floatval', $notes);
        $desc ? rsort($notes) : sort($notes);
    ...
    function setNoteStyle($note, $onlyBad = false)
    {
        $style = '<span class="text';
        if ($note < 10) {
            $style .= '-danger';
