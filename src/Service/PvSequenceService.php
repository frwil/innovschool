...
class PvSequenceService
{
    ...
    public function getSequenceNotes(): array
    {
        ...
        foreach ($schoolClassGrades as $schoolClassGrade) {
            foreach ($schoolClassGrade->getNotesJson() as $studenId => $note) {
                $notes[$studenId][$schoolClassGrade->getSubject()->getId()] = $note;
            }
        }
    ...
    {
        $lowestAverage['lowest'] = min($this->getGeneralAverage());
    ...
    {
        $highestAverage['highest'] = max($this->getGeneralAverage());
